...
figure(2); plot(x_in,output(1:length(x_in)), x_in,outputIdeal); 
...
figure(2); plot(x_in,output(length(x_in)+length(Y_out):end), x_in,outputIdeal2); % Plot first function compared to its expected output 
title("Output Data over Input, sqrt Function (Time Corrected)");
xlabel("Input");
ylabel("Read Address");
legend("Table Output", "Ideal Output");
